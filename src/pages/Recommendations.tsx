...
export function Recommendations() {
  ...
  try {
    // Call Bedrock API with query parameter
    const recs = await getRecommendations(query);
    
    // AI recommendations don't have bookId, so we'll create temporary Book objects
    const tempBooks: Book[] = recs.map((rec, index) => ({
      id: `ai-rec-${index}`,
      ...
      rating: rec.confidence * 5, // Convert confidence to 5-star rating
